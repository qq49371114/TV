...
public class RemoteControlServer extends NanoHTTPD {
    ...
    private static final String ADMIN_PASSWORD = "admin"; // 后台管理员密码
    private boolean isAdminAuthenticated = false; // 登录状态
    ...
    private String getLoginPageHtml(String error) {
        ...
        return "<!DOCTYPE html><html><head><title>远程管理登录</title><meta name='viewport' content='width=device-width, initial-scale=1'><style>body{background:#121212; color:white; font-family:sans-serif; display:flex; justify-content:center; align-items:center; height:100vh; margin:0;} div{background:#212121; padding:30px; border-radius:10px; text-align:center; box-shadow: 0 0 15px rgba(0,0,0,0.5);} input{margin-top:10px; padding:10px; border-radius:5px; border:1px solid #555; background:#333; color:white;} button{margin-top:20px; padding:10px 20px; background:#4CAF50; color:white; border:none; border-radius:5px; cursor:pointer;}</style></head><body><div><h2>请输入管理员密码</h2>" + errorMsg + "<form method='POST'><input type='password' name='admin_password' autofocus/><br/><button type='submit'>登录</button></form></div></body></html>";
    }
    ...
    private String getAdminPanelHtml() {
        ...
        return "<!DOCTYPE html><html><head><title>婉儿守护控制台</title><meta name='viewport' content='width=device-width, initial-scale=1'><style>body{background:#121212; color:white; font-family:sans-serif; text-align:center; padding:20px;} div{background:#212121; border-radius:10px; padding:20px; margin-bottom:20px; max-width:500px; display:inline-block; vertical-align:top; box-shadow: 0 0 15px rgba(0,0,0,0.5);} h1{color:#4CAF50;} h2{color:#2980b9;} ul{list-style:none; padding:0;} li{background:#333; margin-bottom:10px; padding:10px; border-radius:5px; display:flex; justify-content:space-between; align-items:center;} input[type=time],input[type=text]{margin:0 10px; padding:8px; border-radius:5px; border:1px solid #555; background:#333; color:white;} button{cursor:pointer; background:#2980b9; color:white; border:none; padding:8px 15px; border-radius:5px;}</style></head><body><h1>婉儿守护控制台</h1>" +
               "<div><h2>定时锁管理</h2><ul>" + timeSlotsHtml.toString() + "</ul><hr style='border-color:#444;'>" +
               ...
               "<div><h2>密码锁管理</h2><p style='color:#aaa;'>" + passStatus + "</p>" +
